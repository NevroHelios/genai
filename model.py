...
class GenreClassifierCNN(pl.LightningModule):
    def __init__(self, lr:float, label_smoothing:float=CFG.LABEL_SMOOTH,
        ...
        self.train_f1 = F1Score(task='multiclass', average='macro')
        self.val_f1 = F1Score(task='multiclass', average='macro')
        ...
        self.model = nn.Sequential(
            *blocks,
            ...
            nn.Linear(in_features=out_feat, out_features=self.num_classes)
        )
    ...
    def forward(self, x):
        if self.train:
            x = self.time_mask(self.freq_mask(x[:, 0:1, :, :])).view(x)
        return self.model(x)
